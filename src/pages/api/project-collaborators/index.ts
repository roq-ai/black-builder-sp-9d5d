...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getProjectCollaborators() {
    const data = await prisma.project_collaborator
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'project_collaborator'));
    return res.status(200).json(data);
  }

  async function createProjectCollaborator() {
    await projectCollaboratorValidationSchema.validate(req.body);
    const body = { ...req.body };

    const data = await prisma.project_collaborator.create({
      data: body,
    });
    return res.status(200).json(data);
  }
