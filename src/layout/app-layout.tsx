...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['entrepreneur', 'collaborator', 'mentor'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<CollaboratorInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/collaborators` : null),
    () => getCollaborators({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Collaborator',
      path: isTenantUser ? `/collaborators/view/${data?.[0]?.id}` : '/collaborators',
      entity: 'collaborator',
